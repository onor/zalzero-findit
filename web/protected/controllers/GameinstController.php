	...
	public function actionPlay(){
		...
		if($gameInstId == '0'){
			...
			if(!empty($last_active_gameInstId)){
				$gameInstId = $last_active_gameInstId;
				$gameinst_id = $last_active_gameInstId;
			}
		  }
		  
		// if user come with gameinst 0 render blank play board and exit
		if($gameInstId == '0'){
			$param = array('startGame'=>'true','gameinst_id'=>$gameinst_id);
			if($new_user == true){
				$param = array('startGame'=>'false','tutorial'=>'true','gameinst_id'=>$gameinst_id);
			}
		...
		if(isset($gameSeat)){
			$param = array('game'=>$gameSeat,'gameinst_id'=>$gameinst_id);
			if($new_user == true){
				$param = array('game'=>$gameSeat,'tutorial'=>'true','gameinst_id'=>$gameinst_id);
			}
			$this->render('play',$param);
