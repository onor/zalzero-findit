...
class CronCommand extends CConsoleCommand {
	public function run($args) {
		...
		// return cron last runtime and next runtime $get_cron_id = 1
		...
		if($get_completed_games)
		{
			...
			{
				$game_id =  $get_completed_game->zlgameround_gameinst_id;	// gameInstID
				
				$get_users_of_game = get_users_of_game($game_id);			// return the game player with gameseat status accpeted
				$check_the_game_status = check_the_game_status($game_id);	// return the game player count with gameseat status accpeted or resigned
				
				$update_game = Zzgameinst::model()->findByPk($game_id);		// iterating on the game to get the insert data on the user in game summary table
				...
				$update_game->save();	// update game status

				if(count($get_users_of_game) > 0)			// if total accepted gameseat status > 0
				{
					foreach($get_users_of_game as $user_id)
					{
						$total_games = get_total_count_of_games($user_id);			// get total games played by the user
						$won_games = get_total_count_of_won_games($user_id);		// get total games won by the user
						...
						{  // updating data for user if not in database
							...
							// inserting data for user id if not in database
		...
		$get_cron_details->save();		// update core run time
