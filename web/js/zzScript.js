...
define('rightHudController',["../../config/config"], function(config) {
  ...
  updateRightHud = function(event, message) {
    var divElement, gameId, gameSeatID, imgElement, index, rightHUD_theirturn, rightHUD_yourturn, status, urDiv, urInviteAcceptDiv, urInviteDeclineDiv, urInviteDiv, _results;
      ...
      if (document.getElementById("right_hud_" + gameId) === null) {
        ...
        _results.push((function() {
          ...
          for (index in message[gameId].PLSC) {
            ...
            if (parseInt(message[gameId].GS) === 1) {
              _results1.push(rightHUD_yourturn.appendChild(urDiv));
            } else if (parseInt(message[gameId].GS) === 2) {
              _results1.push(rightHUD_theirturn.appendChild(urDiv));
            } else {
        ...
        if (jQuery('#rightHUD-theirturn').find(".userArea").length === 0) {
          if (jQuery('#rightHUD-theirturn').find(".newCarouselblanktile").length === 0) {
            _results.push(jQuery('#rightHUD-theirturn').append('<div class="newCarouselblanktile"></div>'));
          } else {
            _results.push(void 0);
          }
        } else {
          _results.push(jQuery("#rightHUD-theirturn .newCarouselblanktile").remove());
        }
      }
    }
    return _results;
  ...
  createGameDetailsPopup = function(gameDetails, gameId) {
    var el, gameSeatID, gdAllRoundDiv, gdDiv, gdPlaydiv, gdReminddiv, gdReturndiv, gdScoreCardBetsPlacedTd, gdScoreCardNameTd, gdScoreCardSerialNumTd, gdScoreCardTr, gdUserImgDiv, gdUserImgclassName, i, index, lastEl, lastRoundId, remindUsersData, roundId, _i, _score;
    ...
    for (index in gameDetails.PLSC) {
      ...
      gdUserImgclassName = gameDetails.PLRS[gameSeatID].PON === 1 ? 'gdUserImg user_online' : 'gdUserImg';
  ...
  onClickAddRedrict = function(userRecodeDivValue, gameID, remove) {
    var game;
    game = gameID;
    return $(userRecodeDivValue).click(function() {
