...
$.fn.infiniteCarousel = function() {
	...
	return this
			.each(function() {
				var $wrapper = $('> div', this).css('overflow', 'hidden'), $slider = $wrapper
						.find('> ul'), $items = $slider.find('> li'), $single = $items
						.filter(':first'),
				...
				// 1. Pad so that 'visible' number will always be seen,
				// otherwise create empty items
				if (($items.length % visible) != 0) {
					$slider.append(repeat('<li class="empty" />', visible
							- ($items.length % visible)));
					$items = $slider.find('> li');
				}

				// 2. Top and tail the list with 'visible' number of items, top
				// has the last section, and tail has the first
				...
				function gotoPage(page) {
					var dir = page < currentPage ? -1 : 1, n = Math
							.abs(currentPage - page), left = singleWidth * dir
							* visible * n;
				...
				$wrapper
						.after('<a class="arrow back carousel_link">&lt;</a><a class="arrow forward carousel_link">&gt;</a>');
