...
jQuery ->
  ...
  OloGlobals = ->
    ...
    jQuery ->
      #jDocument.bind "room:" + oloGlobals.roomCodes.TIME_LEFT, updateTTL
      #jDocument.bind "room:" + oloGlobals.roomCodes.GAME_DURATION, updateGameDuration
  ...
  OloUnionConnection = (->
    ...
    roomSnapshotListener = (requestID, roomID, occupantCount, observerCount, roomAttrsStr) ->
      ...
      console.log('room attrib recive',arguments[8])
      i = 5
      while i < argLen
    ...
    gameChangeListener = (gameInstIdTemp) ->	# fire when user click on crouse to change game
      console.log 'change game for'+gameInstIdTemp
    ...
    sendDeclinedToServer = (gameSeatId,gameId) ->   # fire when user click decined
    	jQuery('#accept_decline_' + gameId).text('In progress..')
...
#
#    updateRoomPrizeName = ->
#      document.getElementById("gamePrizeName").innerHTML = oloGlobals.roomVars[oloGlobals.roomCodes.ROOM_PRIZE_NAME]
#      document.getElementById("gamePrizeImage").alt = oloGlobals.roomVars[oloGlobals.roomCodes.ROOM_PRIZE_NAME] + " Image"
#      document.getElementById("gamePrizeImage").src = oloGlobals.roomVars[oloGlobals.roomCodes.ROOM_IMAGEROOM_MRP]
#
#    updateRoomPrizeImage = ->
#      document.getElementById("gamePrizeImage").alt = oloGlobals.roomVars[oloGlobals.roomCodes.ROOM_PRIZE_NAME] + " Image"
#      document.getElementById("gamePrizeImage").src = oloGlobals.roomVars[oloGlobals.roomCodes.ROOM_IMAGEROOM_MRP]
#
#    updateRoomPrizeMRP = ->
#      document.getElementById("gamePrizeMRP").innerHTML = "$ " + oloGlobals.roomVars[oloGlobals.roomCodes.ROOM_MRP]
    ...
    jQuery ->
     ...
     # jDocument.bind "room:" + oloGlobals.roomCodes.ROOM_PRIZE_NAME, updateRoomPrizeName
     # jDocument.bind "room:" + oloGlobals.roomCodes.ROOM_IMAGEROOM_MRP, updateRoomPrizeImage
     # jDocument.bind "room:" + oloGlobals.roomCodes.ROOM_MRP, updateRoomPrizeMRP
  ...
  ZalerioGame = (->
    ...
    drawRoundsPanel = ->
      ...
      console.log "roundsLength : ", roundVOs if isDevEnvironment
      cnt = 0
      aHrefElemClone = null
      roundVOsIdx = {}
      for roundId of roundVOs
    ...
    sendPlaceBetToServer = ->
      ...
      playButtonEl = document.getElementById("placeBetOnServer");
      return if playButtonEl.className is "bet_done"
      betStr = ""
      betCtr = 0
...
# message listener        
    messageListener = (event, messageName, broadcastType, fromClientID, roomID, message) ->
      switch messageName
      	when zalerioCMDListners.RIGHT_HUD
	        ...
	        console.log 'MT',usersObject
        	jDocument.trigger "client:" + oloGlobals.clientCodes.RIGHT_HUD,usersObject
        ...
        if e.type is "mouseover" and not isMousingOver
          ...
          if boardVOs[tileIdx]?
            ...
            for playerSeatId of playersObjs
              ...
              if (oloGlobals.userVOsSeatIndex[playerSeatId]?) and (oloGlobals.userVOsSeatIndex[playerSeatId][oloGlobals.clientCodes.USER_FACEBOOK_ID]?)
                imgSrc = "https://graph.facebook.com/" + oloGlobals.userVOsSeatIndex[playerSeatId][oloGlobals.clientCodes.USER_FACEBOOK_ID] + "/picture"
              else
                imgSrc = DEFAULT_PLAYER_IMG_URL
              if oloGlobals.userVOsSeatIndex[playerSeatId]?
                currentFigId = boardVOs[tileIdx][boardVOCodes.FIGURE_ID]
                ...
                strOut += "<li><img src='" + imgSrc + "' alt='" + oloGlobals.userVOsSeatIndex[playerSeatId][oloGlobals.clientCodes.USER_DISPLAY_NAME] + "' ></li>"
  ...
  ZalerioUserComponent = (->
    ...
    onClickMove = (userPanelMov, elemID, direction) ->
      userLiMoveRightAnc = document.createElement("a")
      userLiMoveRightAnc.className = "user" + direction + "MoveAnc"
      userLiMoveRightAnc.id = "id_" + direction + elemID
      # remove old events
      userLiMoveRightAnc.onclick = null
      userLiMoveRightAnc.onclick = ->
        console.log "hiya.. moving " + direction + "_" + elemID if isDevEnvironment
        userPanelMov.move direction, panelMoveMagnitudeCodes.ONE_VIEW

      jQuery("#" + elemID).after userLiMoveRightAnc
    ...
    drawCarousel = (usersObject, appendto, gameLinkID, typeOfCarousel) ->
      ...
      for x of usersObject
        ...
        userRecodeDiv.id = "thisgame_id_"+gameLinkID
            ...
            if oloGlobals.clientVars.UI is userObject.UI
              userRecodePlayerRemindDiv.innerHTML = "place your tiles..."
            	...
            	unless oloGlobals.clientVars.UI is userObject.UI
            		fbUser = []
            		fbUser[0] = userObject.PFB
            		userRecodePlayerRemindDiv.onclick = -> remindUser gameLinkID, fbUser
...
# new logic implimentation on 16 Aug 2012
    	...
    	for index of gameDetails.PLSC
          ...
          continue
    		...
    		if gameDetails.PLRS[gameSeatID].PON is 1
    			gdUserImgDiv.className = 'gdUserImgDiv user_online' 
    ...
    updateRightHud = (event, message)->
    	console.log "updateRightHud data", message
...
#        @currentNoOfUsers = cnt
    ...
    updateZalerioScores = (evt, val) ->
      ...
      showFinalScore userScoreObj, userScoreArr
      refreshUserPanel()
    ...
    showFinalScore = (userScoreObj, userScoreArr) ->
      try
        if oloGlobals.roomVars.FR is "1" # if Game finished
          ...
          allUserData = jQuery.parseJSON(oloGlobals.roomVars.AP)
          console.log "allUserData winner screen", allUserData if isDevEnvironment
          for userSeatIdIndex of userScoreArr
            for x of allUserData
              userInfoObject = jQuery.parseJSON(allUserData[x])
              userInfoObjectPLRS = jQuery.parseJSON(userInfoObject.PLRS)              
              for y of userInfoObjectPLRS
                userInfo = jQuery.parseJSON(userInfoObjectPLRS[y])
                if parseInt(userInfo.PRE) is 1 or parseInt(userInfo.GSS) is 4
                	if parseInt(oloGlobals.currentUserDBId) is parseInt(userInfo.UI)
                		disableRematch = true
                	continue
                if y > 0
                  if y is userScoreArr[userSeatIdIndex]
                    if i is 0
                      rankHtmlArray.push "<div  id='score" + ++i + "'><img src='https://graph.facebook.com/" + userInfo.PFB + "/picture' /><div class='name'>" + userInfo.PDN + "</div><div class='score'>" + userScoreObj[userScoreArr[userSeatIdIndex]] + "</div></div>"
                    else
                      rankHtml += "<div class='score_rep' id='score" + ++i + "'><div class='rank'>" + i + "nd</div><img src='https://graph.facebook.com/" + userInfo.PFB + "/picture' /><div class='name'>" + userInfo.PDN + "</div><div class='score'>" + userScoreObj[userScoreArr[userSeatIdIndex]] + "</div></div>"
          if i < 2 or disableRematch or resignStatus is 1
            jQuery("#rematch").hide();
          ...
          if isDevEnvironment
          	console.log "userinfo", oloGlobals.userVOs
          	console.log "userDivCodes", userDivCodes
    ...
    showRoundScorePopup = () ->
      ...
      for index of usersObject.PLSC
          ...
          continue if usersObject.PLRS[seatId].GSS is 3 or usersObject.PLRS[seatId].GSS is 5
          userCount++
          if userCount <= 3 
    ...
    showInviteStatus = (currentGameUsersData) ->
    	...
    	for x of currentGameUsersData
    		usersInfoObject = jQuery.parseJSON(currentGameUsersData[x])
    ...
    setPlayersInfo = ->
    	currentGameUsersData = jQuery.parseJSON(oloGlobals.roomVars.AP)
    	showInviteStatus(currentGameUsersData) # Function call to show current game invite player status
...
#      jDocument.bind oloEvents.CLIENT_BETS_PLACED, updateBetsPlaced
      ...
      #jDocument.bind "client:" + oloGlobals.clientCodes.MY_TURN, updateRightHudYourTurn
      #jDocument.bind "client:" + oloGlobals.clientCodes.THEIR_TURN, updateRightHudTheirTurn
      #jDocument.bind "client:" + oloGlobals.clientCodes.ALL_PAST_GAME, updateRightHudPastGame
      jDocument.bind "client:" + oloGlobals.clientCodes.RIGHT_HUD,updateRightHud
      jDocument.bind "client:" + oloGlobals.clientCodes.RIGHT_HUD, usersRecordCall
      #jDocument.bind "client:" + oloGlobals.clientCodes.MY_TURN, usersRecordCall
      #jDocument.bind "client:" + oloGlobals.clientCodes.THEIR_TURN, usersRecordCall
  ...
  rematchCall = (e,rematchPlayerFBID)->
  	...
  	if typeof rematchPlayerFBID is "undefined"
  		...
  		for x of usersInfoObject
  			...
  			continue if parseInt(userObject.UI) is parseInt(oloGlobals.currentUserDBId)
  			_results.push(userObject.PFB)
  	else
  		_results = []
  		_results.push(rematchPlayerFBID)
  	InviteFriends(_results)
  	return
  ...
  usersRecordCall = (event,message)->
    usersInfo = 
      RH : message
      APG : oloGlobals.clientVars.APG
      UINFO: oloGlobals.clientVars.UINFO
  ...
  resignPopUP = ->
      jQuery("#bottomHUDbuttons-more").click(->
        All_Player = jQuery.parseJSON(oloGlobals.roomVars.AP);
        for x of All_Player
            userObj = jQuery.parseJSON(All_Player[x])
            totalPlayer = userObj.TP
            break
        currentUserObj = jQuery.parseJSON(oloGlobals.clientVars.UINFO)
...
messagePopup = (message)->
  	...
  	msgDiv.className = 'msg-outer-div'
